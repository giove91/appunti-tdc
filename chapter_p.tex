...
\begin{proof}
 ...
 \begin{itemize}
  ...
  il fatto che $M$ sia in $\ASPACE(c \ln(n))$;
 ...
 \end{itemize}
  ...
  definita come
  \[s(\ID) \equiv (p_1=1 \land p_2 = q_{init} \land p_3=0 \land \overrightarrow{r}=0 \land \overrightarrow{w}=0),\]
  e similmente $t$.
  Concentriamoci dunque su $E^A$: dobbiamo
  fare in modo che valga $E(\ID, \ID')$ se e solo se nel grafo delle
  computazioni di $M$ con input $\bin(A)$ c'è un arco tra la configurazione
  corrisponente a $\ID$ quella corrispondente a $\ID'$.
  Per farlo, ad ogni regola $R$ della macchina di Turing $M$ nella forma
...
\begin{osservazione}
  ...
  polinomiale in $n$ (e quindi il numero di possibili configurazioni non è limitato da un polinomio).
