...
\end{teorema}
\begin{proof}
 Sia $\A$ un problema in $\NP$. Per il Corollario \ref{cor:caratterizzazione-np}
 esiste una macchina di Turing $M$ in $P$ e un $k \in \N$ tale che
 $A \in \A$ se e solo se $M(\bin(A)\string^\bin(R))\accepts$ per una qualche
 ...
 $A,R \models \phi$. Dunque vale $A \in \A$ se e solo se
