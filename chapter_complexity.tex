...
\begin{osservazione}
  ...
  Allora le condizioni su $M$ nell'enunciato della Proposizione~\ref{prop:caratterizzazione-np} possono essere cambiate come segue:
  \begin{itemize}
    \item la macchina $M$ su input $\bin(A)\string^ c$ termina in tempo polinomiale rispetto a $|\bin(A)|$, per ogni stringa binaria $c$ di lunghezza polinomiale rispetto a $|\bin(A)|$;
    \item per ogni $L$-struttura $A$, si ha che $A\in S$ se e solo se esiste una stringa binaria $c$, di lunghezza polinomiale in $|\bin(A)|$, tale che $M(\bin(A)\string^ c)\downarrow$.
  \end{itemize}
\end{osservazione}
